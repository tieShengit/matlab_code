...
% Calculate the registered MID (in ratio form) for each metabolite at each time point
...
for met = 1:length(mets_name)
    ...
    for t = 1:6
        ...
        % Read each mid, calculate the ratio
        for i = 1:length(mids)
            sheet_name = mids{i};
        ...
        % Save the label for each spatial point
